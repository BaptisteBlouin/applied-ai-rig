...
class ClassificationTests(unittest.TestCase):
    ...
    def test_existing_agent_file_becomes_manual_integration(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            plan = build_plan(target, core_profile(), ROOT / "applied_ai_rig" / "templates")
    ...
    def test_profile_json_is_valid_and_contains_selected_modules(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            plan = build_plan(Path(directory), profile, ROOT / "applied_ai_rig" / "templates")
...
class RerunTests(unittest.TestCase):
    def test_unchanged_rerun_has_only_unchanged_files(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            target = Path(directory)
            first = build_plan(target, core_profile(), ROOT / "applied_ai_rig" / "templates")
            ...
            second = build_plan(target, core_profile(), ROOT / "applied_ai_rig" / "templates", checksums)
